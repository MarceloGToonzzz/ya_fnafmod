...
public class ShadowBonnieEntity extends Monster implements GeoEntity {
	...
	public ShadowBonnieEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(YaFnafmodModEntities.SHADOW_BONNIE.get(), world);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "sbonnie");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(6, new RandomStrollGoal(this, 2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = ShadowBonnieEntity.this.getRandom();
				double dir_x = ShadowBonnieEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ShadowBonnieEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ShadowBonnieEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) && this.onGround()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.bonnie.sbonnie_fly"));
			}
			if (!this.onGround()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.bonnie.sbonnie_fly"));
			}
