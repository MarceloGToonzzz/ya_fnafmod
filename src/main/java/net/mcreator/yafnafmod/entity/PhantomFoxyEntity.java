...
public class PhantomFoxyEntity extends Monster implements GeoEntity {
	...
	public PhantomFoxyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(YaFnafmodModEntities.PHANTOM_FOXY.get(), world);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "pfoxy");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = PhantomFoxyEntity.this.getRandom();
				double dir_x = PhantomFoxyEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = PhantomFoxyEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = PhantomFoxyEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			...
			if (!this.onGround()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.foxy.pfoxy_fly"));
			}
