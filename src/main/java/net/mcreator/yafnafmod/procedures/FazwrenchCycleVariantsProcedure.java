	...
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == YaFnafmodModItems.FAZWRENCH.get()) {
			if (blockstate.getBlock() == YaFnafmodModBlocks.COAT_HANGER.get()) {
				...
				variant = 5;
			} else if (blockstate.getBlock() == YaFnafmodModBlocks.WALL_PIZZA.get() || blockstate.getBlock() == YaFnafmodModBlocks.WALL_PIZZA_OLIVE.get()) {
				variant = 2;
			}
			if ((blockstate.getBlock().getStateDefinition().getProperty("blockstate") instanceof IntegerProperty _getip25 ? blockstate.getValue(_getip25) : -1) != variant) {
				{
